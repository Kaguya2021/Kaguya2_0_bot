...
const replyModeCache = new Map();      // НОВОЕ: Кэш для хранения режима ответов (once/always)
...
bot.command('start', async (ctx) => {
  ...
  const welcomeText = 
    ...
    '🔄 <b>Режим ответов:</b> <code>/mode</code> (Отвечать 1 раз или всегда)'; // НОВОЕ
...
// --- НОВОЕ: КОМАНДА /mode (Настройка частоты ответов) ---
bot.command('mode', async (ctx) => {
  const userId = String(ctx.from.id);
  const args = ctx.message.text.trim().toLowerCase().split(/\s+/);
  const newMode = args[1];

  if (newMode === 'once') {
    replyModeCache.set(userId, 'once');
    if (db.setReplyMode) db.setReplyMode(userId, 'once').catch(() => {}); // Сохраняем в БД, если есть метод
    return await ctx.reply('✅ <b>Режим обновлен:</b> Теперь бот будет отвечать клиенту <b>только 1 раз</b> (следующий автоответ сработает не раньше чем через 24 часа).', { parse_mode: 'HTML' });
  } else if (newMode === 'always') {
    replyModeCache.set(userId, 'always');
    if (db.setReplyMode) db.setReplyMode(userId, 'always').catch(() => {}); // Сохраняем в БД, если есть метод
    return await ctx.reply('✅ <b>Режим обновлен:</b> Теперь бот будет отвечать на <b>каждое сообщение</b> (с анти-спам задержкой 3 секунды).', { parse_mode: 'HTML' });
  }

  // Если аргументы не переданы, показываем текущий статус
  ...
  const statusText = currentMode === 'once' ? '1 раз (once)' : 'На каждое сообщение (always)';
  await ctx.reply(
    `🔄 <b>Текущий режим ответов:</b> ${statusText}\n\n` +
    'Чтобы изменить, используйте команды:\n' +
    '<code>/mode once</code> — Бот ответит только один раз при первом обращении.\n' +
    '<code>/mode always</code> — Бот будет отвечать на каждое новое сообщение.', 
    { parse_mode: 'HTML' }
  );
});
...
bot.on('business_message', async (ctx) => {
  try {
    ...
    // 3. ПРОВЕРКА ПАУЗ И РЕЖИМА ОТВЕТА (НОВОЕ)
    ...
    // --- НОВОЕ: ПРОВЕРКА ВЫБРАННОГО РЕЖИМА ПЕРЕД ОТПРАВКОЙ ---
    ...
    if (ownerId) {
       ...
       if (!currentMode && db.getReplyMode) {
           ...
           currentMode = 'always'; // Дефолт, если БД не поддерживает
       }
    }

    // Устанавливаем паузу в зависимости от режима:
    if (currentMode === 'once') {
       // Блокируем автоответы для этого чата на 24 часа (или до ответа владельца)
       localPauses.set(chatId, Date.now() + 24 * 60 * 60 * 1000);
    } else {
       // Стандартный анти-спам (3 секунды)
    ...
    try {
      if (replyText.startsWith('combo:')) {
        ...
        if (parts[0]) await ctx.api.sendMessage(chatId, parts[0], { business_connection_id: connec
